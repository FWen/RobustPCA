...
X = double(rgb2gray(imread('turtle.png')));
Y = double(rgb2gray(imread('masked_turtle.png')));%corrupted by text
[m,n] = size(X);
...
figure(2);
contourf(qs,qs,PSNR,[v0:1:v1]); colorbar; xlabel('q_2'); ylabel('q_1');
set(gca, 'CLim', [v0, v1]);
...
title(sprintf('BCD (best, q1=%.1f, q2=%.1f)\n RelErr=%.3f, PSNR=%.2f dB',qs(ko),qs(lo),RelErrs(ko,lo),PSNR(ko,lo)));

figure(1);subplot(1,4,3);
imshow(uint8(reshape(x_Lq(:,1,1),[m,n])));
title(sprintf('Hard\n RelErr=%.3f, PSNR=%.2f dB',RelErrs(1,1),PSNR(1,1)));
